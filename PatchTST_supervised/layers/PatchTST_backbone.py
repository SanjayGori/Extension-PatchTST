...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pe == 'zeros' and self.learn_pe:
            self.pos_embed = nn.Parameter(torch.zeros(1, patch_num, self.d_model, device=x.device))
        ...
        self.head = Flatten_Head(self.individual, self.n_vars, x.shape[2] * x.shape[3], self.target_window, head_dropout=self.head_dropout).to(x.device)
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def set_patch_num(self, patch_num):  # for dynamic patching
        self.seq_len = patch_num
        self.patch_num = patch_num
        self.W_pos = positional_encoding('zeros', True, patch_num, self.W_pos.shape[-1])
